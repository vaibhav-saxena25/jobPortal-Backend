...
export const updateJobController = async (req, res, next) => {
    ...
    if (!job) {
        return next(new Error(`No jobs found with ID ${id}`));
    }
...
export const jobStatsController = async (req, res) => {
    const stats = await jobsModel.aggregate([
      ...
      // Group by job status and count the number of jobs for each status
      {
        $group: {
          _id: "$status", // Group by the 'status' field (e.g., active, closed)
          count: { $sum: 1 }, // Count the number of jobs in each status group
        },
      },
    ]);

    // If no stats are found, return an default stats
    const defaultStats = {
    ...
    // Return the total number of jobs and the stats (grouped by status)
